...
import { ChannelActivity, TenantRequest, UserLocation, UserInfo } from '../types/channel.types';
import { UserLocation as DBUserLocation, TenantRequest as DBTenantRequest } from '@/lib/services/indexedDBSchema';
    ...
    try {
      ...
      pendingFetchPromise.current = (async () => {
        try {
          ...
          const response = await fetch(`/api/myinfo?userId=${userId}`, {
            method: 'POST',
            headers: {
  ...
  async function signIn(email: string, password: string) {
    ...
    if (data.user) {
      try {
        ...
        const response = await fetch(`/api/myinfo?userId=${data.user.id}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Cache-Control': 'max-age=300',
