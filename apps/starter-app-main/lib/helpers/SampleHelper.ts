...
export function SampleHelper(): SampleHelperReturn {
  ...
  const followChannel = async (username: string): Promise<void> => {
    ...
    try {
      ...
      if (!isGuest) {
        const endpoint = `/api/follow?username=${encodeURIComponent(username)}&userId=${encodeURIComponent(user.id)}`;
  ...
  const completeChannelOnboarding = async (channelUsername: string, channelDetails: Channel): Promise<boolean> => {
    try {
      ...
      if (channelDetails.is_owner_db) {
        ...
        const response = await fetch(`/api/channels/${channelDetails.username}/request-access`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const fetchChannelMessages = async (
    ...
    try {
      ...
      const response = await fetch(`/api/channels/${username}/messages?${queryParams.toString()}`);
  ...
  const updateChannelLastViewed = async (username: string): Promise<boolean> => {
    try {
      ...
      const response = await fetch(`/api/channels/${username}/last-viewed`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
