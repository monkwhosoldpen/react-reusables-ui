...
export async function fetchTenantUsers(): Promise<TenantUser[]> {
  try {
    const response = await fetch('/api/dashboard/tenant-users');
    if (!response.ok) {
...
export async function fetchTenantRequests(): Promise<TenantRequest[]> {
  try {
    const response = await fetch('/api/dashboard/tenant-requests');
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.error || `HTTP error! status: ${response.status}`);
    }
...
export async function createTenantTables(): Promise<void> {
  try {
    const response = await fetch('/api/dashboard/create-tables', {
      method: 'POST',
    });
